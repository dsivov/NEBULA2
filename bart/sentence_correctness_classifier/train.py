...
def train():
    ...
    for epoch_i in range(0, epochs):
        ...
        if best_eval_accuracy < avg_val_accuracy:
            ...
            # SAVE THE WEIGHTS
            today = datetime.now()
            dir_name = "./bart/sentence_correctness_classifier/weights-" + today.strftime('%Y%m%d')
            if not os.path.exists(dir_name):
                os.mkdir(dir_name)
            model.save_pretrained(os.path.join(dir_name, "weights-" + today.strftime('%Y%m%d%h%m')))
            torch.save(model.state_dict(), './bart/sentence_correctness_classifier/best_model.pt')

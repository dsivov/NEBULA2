...
def read_frames(video_path, fps, frame_size, frame_crop_size, alpha_h, alpha_w, hflip=False, max_frames=None, t_start=None, t_end=None):
    ...
    cmd = f"ffmpeg -y {ss_cmd} -i {video_path}  -max_muxing_queue_size 9999  -loglevel error -vf 'fps={fps}:round=up,scale={scale_w}:{scale_h},crop={crop_w}:{crop_h}:{w0}:{h0}{hflip_filter}' {dframes} {t_cmd} -pix_fmt rgb24 -f rawvideo -nostdin pipe:"
...
def read_frames_center_crop(video_path, fps, frame_size, frame_crop_size, t_start=None, t_end=None):
    return read_frames(video_path, fps, frame_size, frame_crop_size, alpha_h=0.5, alpha_w=0.5, t_start=t_start, t_end=t_end)

def read_frames_center_crop_batch(video_path, fps, frame_size, frame_crop_size, batch_num_frames, t_start=None, t_end=None):
    batch_byte_size = batch_num_frames * frame_crop_size * frame_crop_size * 3
    data0 = b''
    for data in read_frames_center_crop(video_path, fps, frame_size, frame_crop_size, t_start=t_start, t_end=t_end):
        if len(data0) > 0:
            data0 = data0 + data
        else:
            data0 = data
...
def ffmpeg_audio_reader(in_filename, t_start=None, t_end=None):
    if t_start is None:
        assert t_end is None
        ss_cmd = ''
        t_cmd = ''
        ...
        ss_cmd = f'-ss {t_start}'
        t_cmd = f'-t {t_end-t_start}'
    cmd = f'ffmpeg {ss_cmd}  -i {in_filename} -loglevel quiet {t_cmd}  -f wav -nostdin  pipe:'
    p = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE)
    all_data = b''
    while True:
